...
**Status:** Live as of April 2026. Tours running daily since May 2026 with strong review flow (21 Google + 10 TripAdvisor, all 5★ as of May 2026). Canonical URL is `https://www.norwichfreewalkingtours.co.uk`.
...
- [x] **Google Business Profile set up** (2026-04-15). Listing created + completed. Still TBD: link GBP ↔ Search Console + GA4 once Google Maps verification lands.
- [x] **Already-subscribed UX on email capture** (shipped 2026-04-16, commit `18a9815`). `/api/subscribe` now returns `{success: true, alreadySubscribed: true}` on Postgres `23505` unique-violation; `components/EmailCapture.tsx` shows a distinct "Looks like you're already subscribed. See you on tour." message via a new `"already"` status branch. Part B (resubscribe for previously opted-out users) still blocked on the `contacts` table architectural decision in §11.
...
- [~] **M17. Tip-guide anchor band** — STATUS CORRECTED 2026-05-16. Previously marked shipped on 2026-04-17 but `components/TipAnchor.tsx` does NOT exist in the repo (grep confirms). Either reverted or never landed. Re-built on the `/home-v2` experimental branch at `app/home-v2/_components/TipAnchor.tsx` as part of homepage messaging-hierarchy fix. Canonical tip figure across the site is **£10–£20** (matches booking widget footer + HowItWorks step 3 + FAQ "Is it really free?"). When `/home-v2` is approved and merged into `/`, move the component to `components/TipAnchor.tsx` so it's importable from anywhere.
...
- [x] **Testimonials:** real Google reviews populated in `lib/testimonials.ts` (21 reviews, 5.0 average, all 5★ as of May 2026). TripAdvisor at 5.0 from 10 reviews. `aggregateRating` + `review` objects re-added to JSON-LD in `app/layout.tsx` (conditional on `googleReviewStats.count > 0`).
...
- [ ] **T8. Error monitoring** — add Sentry (free tier) or enable Vercel's built-in observability so failed Zoho SMTP / Supabase calls + client-side exceptions surface automatically.
...
- [ ] **T14. Silent email failure on `/api/contact` and `/api/subscribe`** — both routes catch Zoho SMTP errors and still return `{success: true}` to the client (see `app/api/contact/route.ts:112`, `app/api/subscribe/route.ts:128`). `/api/contact` doesn't persist to Supabase at all, so a Zoho outage = lost lead with no trace. Fix in two parts: (a) always insert the enquiry into a Supabase `enquiries` table first (acts as the system of record even if email send fails — same pattern as `subscribers`), (b) return a non-2xx response if SMTP fails so the client can show a retry message. T8 (Sentry) would surface these on its own; do whichever ships first.
...
### Email sending architecture (current, as of 2026-04-16)

| Route | Provider | From | To | Why |
|---|---|---|---|---|
| `/api/contact` | Zoho SMTP | `hello@norwichfreewalkingtours.co.uk` | `hello@norwichfreewalkingtours.co.uk` | Zoho-to-Zoho bypasses 554 5.7.7 self-spoof rule |
| `/api/subscribe` | Zoho SMTP | `hello@norwichfreewalkingtours.co.uk` | `hello@norwichfreewalkingtours.co.uk` | Same reason |
| Booking confirmations | Resend (booking widget repo) | `@norwichfreewalkingtours.co.uk` | Customer email | External inbox — no Zoho self-spoof issue |

SMTP config: `smtp.zoho.eu:465`, SSL. Helper at `lib/zohoMail.ts`. Requires `ZOHO_EMAIL` + `ZOHO_APP_PASSWORD` env vars in Vercel. App password generated at `accounts.zoho.eu` under Application-Specific Passwords — **not** the main account password. Do NOT add Resend back to these routes without also switching the verified Resend sending domain off the root (Zoho-hosted) domain.
